...
function App() {
  ...
  // --- RESTOCK FUNCTION (CONNECTED TO VERCEL BACKEND) ---
  const triggerRestock = async (item, quantity) => {
    ...
    try {
      ...
      // 1. Order (Using Live Backend)
      addLog(`📡 Contacting Supplier Agent...`);
      let response = await fetch(`${BACKEND_URL}/buy-stock`, {
        method: 'POST',
      ...
      if (response.status === 402) {
        ...
        // 3. Verify
        addLog(`📨 Sending Payment Proof...`);
        response = await fetch(`${BACKEND_URL}/buy-stock`, {
          method: 'POST',
